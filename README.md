# Diego-Illobre

# Hi! I'm Diego 👋

### 🚀 About Me
- 📊 **Data Analyst** passionate about insights and visualization.
- 🎓 Currently training to master the data stack.
- 🎨 Combining technical rigor with a creative approach.

### 🛠️ Tech Stack & Tools
- **Languages:** Python (Pandas, Numpy), SQL.
- **Visualization:** Tableau, Power BI.
- **Environments:** Jupyter Notebooks, GitHub.

### 📈 Featured Projects
- Guided Walkthrough: Regional Profitability & Product Growth([link-to-repo](https://github.com/VizTraveler/AdventureWorks-Executive-Sales-Performance):
-  Analyzing using SQL Server Management System and Power BI

---
📫 **Connect with me:** 
www.linkedin.com/in/viztraveler
https://ethanlab7.wordpress.com/

